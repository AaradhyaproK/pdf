...
export default function CompressImageTo50KBPage() {
  ...
  const handleCompress = async () => {
    ...
    try {
      const res = await compressImageToTargetKB(files[0].file, 50);
      setResultFile(res);
      setDownloadUrl(URL.createObjectURL(res));
      toast.success('Image compressed under 50KB!');
    } catch (err: any) {
  ...
  return (
    <ToolLayout
      ...
      subtitle="Reduce photo file size under 50KB for passport portals, visa applications, and student forms. 100% Client-Side."
    >
      <div className="space-y-6">
        <FileUploader
          ...
          title="Upload image to compress under 50KB"
        />
        ...
        {files.length > 0 && (
          ...
          >
            {isProcessing ? 'Optimizing under 50KB...' : 'Compress to < 50KB'}
          </button>
        )}
