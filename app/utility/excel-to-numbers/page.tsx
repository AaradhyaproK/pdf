...
export default function ExcelToNumbersPage() {
  ...
  const handleConvert = async () => {
    ...
    setProgressMessage('Reading spreadsheet workbooks & worksheets...');

    try {
      const result = await convertExcelToNumbers(file, (percent, message) => {
    ...
    >
      <div className="space-y-8">
        ...
        <div className="bg-slate-100/90 p-1.5 rounded-2xl flex items-center justify-center max-w-xl mx-auto border border-slate-200/80 shadow-2xs">
          <div className="flex-1 py-2.5 px-3 sm:px-4 rounded-xl bg-white text-emerald-700 font-extrabold text-xs sm:text-sm shadow-xs flex items-center justify-center gap-1.5 sm:gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
            <span>Excel (.xlsx) → Numbers (.numbers)</span>
          </div>

          <Link
            href="/utility/numbers-to-excel"
          ...
          >
            <ArrowLeftRight className="w-3.5 h-3.5 text-slate-400 group-hover:text-indigo-600 transition-transform group-hover:rotate-180" />
            <span className="hidden xs:inline">Numbers → Excel</span>
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-b from-white to-slate-50/80 border border-slate-200/90 p-6 sm:p-8 shadow-sm">
          <div className="absolute top-0 right-0 -mr-16 -mt-16 w-56 h-56 bg-emerald-400/10 rounded-full blur-3xl pointer-events-none" />
          <div className="absolute bottom-0 left-0 -ml-16 -mb-16 w-56 h-56 bg-amber-400/10 rounded-full blur-3xl pointer-events-none" />
          ...
          <div className="relative flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6 max-w-2xl mx-auto">
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto p-4 rounded-2xl bg-white border border-emerald-100 shadow-2xs">
              ...
              <div>
                <span className="text-[10px] font-black uppercase tracking-wider text-emerald-700 block">Source Format</span>
                <h3 className="text-base font-black text-slate-900">Microsoft Excel</h3>
                <span className="text-xs text-slate-500 font-medium">.xlsx, .xls, .csv</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto p-4 rounded-2xl bg-white border border-amber-100 shadow-2xs">
              ...
              <div>
                <span className="text-[10px] font-black uppercase tracking-wider text-amber-700 block">Target Format</span>
                <h3 className="text-base font-black text-slate-900">Apple Numbers</h3>
                <span className="text-xs text-slate-500 font-medium">.numbers package</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 sm:gap-3">
          ...
          <div className="p-3 sm:p-4 rounded-2xl bg-white border border-slate-200/80 shadow-2xs flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-medium">Works on Windows PC</p>
            </div>
          </div>
          ...
          <div className="p-3 sm:p-4 rounded-2xl bg-white border border-slate-200/80 shadow-2xs flex items-center gap-2.5">
            ...
            <div>
              <h4 className="text-xs font-black text-slate-900">No Sign-Up</h4>
        ...
        <div className="space-y-4">
          <FileUploader
            ...
            subtitle="Drag & drop or tap to select spreadsheet from your computer or phone."
          />
          ...
          {files.length > 0 && !downloadUrl && (
            <div className="space-y-3">
              <button
                ...
                className="w-full py-4 rounded-2xl bg-emerald-600 hover:bg-emerald-700 active:scale-[0.98] text-white font-black text-sm sm:text-base shadow-xl shadow-emerald-600/20 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
              >
                {isProcessing ? (
                  <>
                    <Sparkles className="w-4 h-4 animate-spin" />
                    <span>Converting Excel to Numbers...</span>
                  </>
                ) : (
                  <>
                    <Table className="w-5 h-5" />
                    <span>Convert to Apple Numbers (.numbers)</span>
                  </>
                )}
              </button>
                ...
                <div className="p-4 rounded-2xl bg-slate-900 text-white space-y-2.5 shadow-lg border border-slate-800">
                  <div className="flex items-center justify-between text-xs font-bold">
                    <span className="flex items-center gap-2 text-emerald-400">
                      ...
                      {progressMessage || 'Compiling Apple Numbers package...'}
                    </span>
              ...
              <div className="flex items-center gap-3 text-left w-full sm:w-auto">
                ...
                <div>
                  <h4 className="font-black text-sm sm:text-base text-slate-900 flex items-center gap-1.5">
                    <span>Apple Numbers Package (.numbers) Ready!</span>
                  </h4>
        ...
        {/* Technical Deep Dive: Why Convert Excel to Apple Numbers? */}
        <section className="space-y-4">
          <div className="text-center max-w-2xl mx-auto space-y-1">
            <h2 className="text-lg sm:text-2xl font-black text-slate-900">
              Understanding Apple Numbers & Microsoft Excel Architecture
            </h2>
            <p className="text-xs sm:text-sm text-slate-500 font-medium">
              Why spreadsheet files behave differently across Mac, iOS, and Windows devices.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-5 rounded-2xl sm:rounded-3xl bg-white border border-slate-200/90 shadow-2xs space-y-2">
              ...
              <h3 className="text-sm font-black text-slate-900">The OpenXML Grid System</h3>
              <p className="text-xs text-slate-600 leading-relaxed font-medium">
                Microsoft Excel workbooks (.xlsx) use the OpenXML compressed archive format. Worksheets consist of a rigid, infinite grid of 1,048,576 rows by 16,384 columns, where every element must exist inside cell coordinates.
              </p>
            </div>
            ...
            <div className="p-5 rounded-2xl sm:rounded-3xl bg-white border border-slate-200/90 shadow-2xs space-y-2">
              ...
              <h3 className="text-sm font-black text-slate-900">The Apple Freeform Canvas</h3>
              <p className="text-xs text-slate-600 leading-relaxed font-medium">
                Apple Numbers departs entirely from single-grid spreadsheets. Instead of an infinite grid, Numbers sheets are flexible graphical canvases that host multiple independent tables, charts, and media blocks side-by-side.
              </p>
            </div>
            ...
            <div className="p-5 rounded-2xl sm:rounded-3xl bg-white border border-slate-200/90 shadow-2xs space-y-2">
              ...
              <h3 className="text-sm font-black text-slate-900">FileZenith Local Package Engine</h3>
              <p className="text-xs text-slate-600 leading-relaxed font-medium">
                Our engine parses sheet dimensions, cell datatypes, and formula results, structuring them into a valid Apple Numbers package containing vector previews and metadata that Apple iWork opens without friction.
              </p>
            </div>
          </div>
        </section>
            ...
            <table className="w-full text-left border-collapse text-xs sm:text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50">
                  ...
                  <th className="p-3 sm:p-4 font-black text-emerald-700">FileZenith Engine</th>
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                <tr className="hover:bg-slate-50/60">
                  ...
                  <td className="p-3 sm:p-4 text-emerald-700 font-bold">.xlsx, .xls, .csv</td>
                  <td className="p-3 sm:p-4 text-slate-600">Microsoft Excel 97-2024, Office 365, Google Sheets export, CSV dumps</td>
                </tr>
                <tr className="hover:bg-slate-50/60">
                  <td className="p-3 sm:p-4 font-bold text-slate-900">Generated Output Format</td>
                  <td className="p-3 sm:p-4 text-emerald-700 font-bold">.numbers (Apple iWork)</td>
                  <td className="p-3 sm:p-4 text-slate-600">Native package recognized on Mac, iPad, iPhone, and iCloud Numbers</td>
                </tr>
                <tr className="hover:bg-slate-50/60">
                  <td className="p-3 sm:p-4 font-bold text-slate-900">Client Platform Support</td>
                  <td className="p-3 sm:p-4 text-emerald-700 font-bold">Universal (Web Browser)</td>
                  <td className="p-3 sm:p-4 text-slate-600">Windows 11/10, macOS, Linux, ChromeOS, Android, iOS</td>
                </tr>
                <tr className="hover:bg-slate-50/60">
                  <td className="p-3 sm:p-4 font-bold text-slate-900">Cloud Server Upload</td>
                  <td className="p-3 sm:p-4 text-emerald-700 font-bold">0 KB (Zero Uploads)</td>
                  <td className="p-3 sm:p-4 text-slate-600">100% Client-Side memory execution; compliant with strict privacy mandates</td>
                </tr>
                <tr className="hover:bg-slate-50/60">
                  <td className="p-3 sm:p-4 font-bold text-slate-900">Apple ID / iCloud Requirement</td>
                  <td className="p-3 sm:p-4 text-emerald-700 font-bold">None Required</td>
                  <td className="p-3 sm:p-4 text-slate-600">Generate Apple Numbers spreadsheets without needing an iCloud or Apple account</td>
                </tr>
              </tbody>
            </table>
          </div>
