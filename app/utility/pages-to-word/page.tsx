...
export default function PagesToWordPage() {
  ...
  const handleConvert = async () => {
    ...
    try {
      const result = await convertPagesToDocx(file);
      ...
      toast.success('Pages document successfully converted to Word (.docx)!');
    } catch (err: any) {
  ...
  return (
    ...
    >
      <div className="space-y-5">
        <FileUploader
          accept=".pages,application/x-iwork-pages-sffpages,application/vnd.apple.pages"
          multiple={false}
          files={files}
          onFilesSelected={handleFilesChange}
          onRemoveFile={handleReset}
          title="Upload Apple Pages (.pages) document to convert to Word"
          subtitle="Drag & drop or tap to browse Pages document from your device."
        />
          ...
          >
            {isProcessing ? (
              <>
                ...
                <span>Converting Pages to Word (.docx)...</span>
              </>
            ) : (
              <>
                <FileText className="w-5 h-5" />
                <span>Convert Pages to Word (.docx)</span>
              </>
            )}
          </button>
        )}
