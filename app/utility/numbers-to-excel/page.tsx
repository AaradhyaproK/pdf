...
export default function NumbersToExcelPage() {
  ...
  const handleConvert = async () => {
    ...
    setProgressMessage('Inspecting Apple Numbers container package...');

    try {
      const result = await convertNumbersToExcel(file, (percent, message) => {
    ...
    >
      <div className="space-y-8">
        ...
        <div className="bg-slate-100/90 p-1.5 rounded-2xl flex items-center justify-center max-w-xl mx-auto border border-slate-200/80 shadow-2xs">
          ...
          >
            <ArrowLeftRight className="w-3.5 h-3.5 text-slate-400 group-hover:text-emerald-600 transition-transform group-hover:rotate-180" />
            <span className="hidden xs:inline">Excel → Numbers</span>
          ...
          <div className="flex-1 py-2.5 px-3 sm:px-4 rounded-xl bg-white text-indigo-700 font-extrabold text-xs sm:text-sm shadow-xs flex items-center justify-center gap-1.5 sm:gap-2">
            ...
            <span>Numbers (.numbers) → Excel (.xlsx)</span>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-b from-white to-slate-50/80 border border-slate-200/90 p-6 sm:p-8 shadow-sm">
          <div className="absolute top-0 right-0 -mr-16 -mt-16 w-56 h-56 bg-amber-400/10 rounded-full blur-3xl pointer-events-none" />
          <div className="absolute bottom-0 left-0 -ml-16 -mb-16 w-56 h-56 bg-emerald-400/10 rounded-full blur-3xl pointer-events-none" />
          ...
          <div className="relative flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6 max-w-2xl mx-auto">
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto p-4 rounded-2xl bg-white border border-amber-100 shadow-2xs">
              ...
              <div>
                <span className="text-[10px] font-black uppercase tracking-wider text-amber-700 block">Source Format</span>
                <h3 className="text-base font-black text-slate-900">Apple Numbers</h3>
                <span className="text-xs text-slate-500 font-medium">.numbers spreadsheet</span>
              </div>
            </div>

            {/* Middle Pipeline Arrow */}
            <div className="flex flex-col items-center justify-center shrink-0">
              <div className="p-3 rounded-full bg-emerald-50 text-emerald-600 border border-emerald-100 shadow-2xs">
            ...
            {/* Target: Excel */}
            <div className="flex items-center gap-3 w-full sm:w-auto p-4 rounded-2xl bg-white border border-emerald-100 shadow-2xs">
              <div className="w-12 h-12 rounded-xl bg-emerald-600 text-white flex items-center justify-center shrink-0 shadow-xs">
                <FileSpreadsheet className="w-6 h-6" />
              </div>
              <div>
                <span className="text-[10px] font-black uppercase tracking-wider text-emerald-700 block">Target Format</span>
                <h3 className="text-base font-black text-slate-900">Microsoft Excel</h3>
                <span className="text-xs text-slate-500 font-medium">.xlsx OpenXML Workbook</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 sm:gap-3">
          ...
          <div className="p-3 sm:p-4 rounded-2xl bg-white border border-slate-200/80 shadow-2xs flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-medium">No iCloud needed</p>
            </div>
          </div>
          ...
          <div className="p-3 sm:p-4 rounded-2xl bg-white border border-slate-200/80 shadow-2xs flex items-center gap-2.5">
            ...
            <div>
              <h4 className="text-xs font-black text-slate-900">Instant Convert</h4>
              <p className="text-[10px] text-slate-500 font-medium">In-browser processing</p>
            </div>
          </div>
          ...
          <div className="p-3 sm:p-4 rounded-2xl bg-white border border-slate-200/80 shadow-2xs flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-medium">No registration required</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <FileUploader
            accept=".numbers,application/vnd.apple.numbers,application/x-iwork-numbers-sffnumbers,application/zip"
            multiple={false}
            files={files}
            onFilesSelected={handleFilesChange}
            onRemoveFile={handleReset}
            title="Upload Apple Numbers (.numbers) spreadsheet to convert to Excel"
            subtitle="Drag & drop or tap to select .numbers file from your Mac, PC, or phone."
          />
              ...
              >
                {isProcessing ? (
                  <>
                    ...
                    <span>Converting Numbers to Excel...</span>
                  </>
                ) : (
                  <>
                    <FileSpreadsheet className="w-5 h-5" />
                    <span>Convert to Microsoft Excel (.xlsx)</span>
                  </>
                )}
              </button>
                ...
                <div className="p-4 rounded-2xl bg-slate-900 text-white space-y-2.5 shadow-lg border border-slate-800">
                  <div className="flex items-center justify-between text-xs font-bold">
                    <span className="flex items-center gap-2 text-indigo-400">
                      ...
                      {progressMessage || 'Compiling Microsoft Excel (.xlsx) workbook...'}
                    </span>
                    <span className="font-mono text-indigo-300">{progressPercent}%</span>
                  </div>
                  <div className="w-full h-2 rounded-full bg-slate-800 overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-indigo-500 via-emerald-400 to-emerald-500 transition-all duration-300 rounded-full"
            ...
            <div className="p-5 sm:p-6 rounded-2xl sm:rounded-3xl bg-gradient-to-r from-emerald-500/10 via-emerald-500/15 to-emerald-500/10 border border-emerald-500/30 text-emerald-950 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-md animate-in fade-in duration-200">
              <div className="flex items-center gap-3 text-left w-full sm:w-auto">
                ...
                <div>
                  <h4 className="font-black text-sm sm:text-base text-slate-900 flex items-center gap-1.5">
                    <span>Microsoft Excel Workbook (.xlsx) Ready!</span>
                  </h4>
                  <p className="text-xs text-slate-600 font-medium">
                    {stats
                      ? `${stats.sheetCount} sheet(s) • ${stats.rowCount.toLocaleString()} rows${stats.fileSizeKB ? ` • ${stats.fileSizeKB} KB` : ''}`
              ...
              <div className="flex items-center gap-2.5 w-full sm:w-auto">
                <button
                  ...
                  title="Convert another Numbers spreadsheet"
                >
        ...
        {/* Technical Deep Dive: Why Can't Windows Open Apple Numbers Files? */}
        <section className="space-y-4">
          <div className="text-center max-w-2xl mx-auto space-y-1">
            <h2 className="text-lg sm:text-2xl font-black text-slate-900">
              Why Won&apos;t Apple Numbers Files Open on Windows PC?
            </h2>
            <p className="text-xs sm:text-sm text-slate-500 font-medium">
              The technical explanation of why Microsoft Excel rejects .numbers files and how FileZenith solves it.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-5 rounded-2xl sm:rounded-3xl bg-white border border-slate-200/90 shadow-2xs space-y-2">
              ...
              <h3 className="text-sm font-black text-slate-900">Proprietary Protobuf Bundles</h3>
              <p className="text-xs text-slate-600 leading-relaxed font-medium">
                An Apple Numbers file is actually a zipped macOS bundle folder holding compressed <code>.iwa</code> (iWork Archive) files serialized via Google Protocol Buffers with Snappy compression, which Windows Excel has zero built-in ability to read.
              </p>
            </div>
            ...
            <div className="p-5 rounded-2xl sm:rounded-3xl bg-white border border-slate-200/90 shadow-2xs space-y-2">
              ...
              <h3 className="text-sm font-black text-slate-900">The &ldquo;Rename to .ZIP&rdquo; Myth</h3>
              <p className="text-xs text-slate-600 leading-relaxed font-medium">
                Older tech blogs recommend renaming <code>.numbers</code> to <code>.zip</code>. While this occasionally reveals a preview PDF in older iWork 09 files, modern Numbers 13+ files contain raw binary tables that still won&apos;t open in Excel without real conversion.
              </p>
            </div>
            ...
            <div className="p-5 rounded-2xl sm:rounded-3xl bg-white border border-slate-200/90 shadow-2xs space-y-2">
              ...
              <h3 className="text-sm font-black text-slate-900">True OpenXML Reconstruction</h3>
              <p className="text-xs text-slate-600 leading-relaxed font-medium">
                FileZenith safely inspects the Numbers package in local browser memory, extracts table records, sheet names, and values, and compiles them into a standard OpenXML <code>.xlsx</code> workbook that opens smoothly across Windows, Office 365, and Google Sheets.
              </p>
            </div>
          </div>
        </section>
            ...
            <table className="w-full text-left border-collapse text-xs sm:text-sm">
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                <tr className="hover:bg-slate-50/60">
                  ...
                  <td className="p-3 sm:p-4 text-indigo-700 font-bold">.numbers</td>
                  <td className="p-3 sm:p-4 text-slate-600">Apple Numbers packages created on macOS, iOS (iPad/iPhone), or iCloud Numbers</td>
                </tr>
                <tr className="hover:bg-slate-50/60">
                  <td className="p-3 sm:p-4 font-bold text-slate-900">Generated Output Format</td>
                  <td className="p-3 sm:p-4 text-indigo-700 font-bold">.xlsx (Microsoft Excel)</td>
                  <td className="p-3 sm:p-4 text-slate-600">Standard OpenXML format recognized by Microsoft Excel, Google Sheets, LibreOffice Calc</td>
                </tr>
                <tr className="hover:bg-slate-50/60">
                  ...
                  <td className="p-3 sm:p-4 text-slate-600">Open Numbers spreadsheets instantly on Windows 11/10 without requiring a Mac</td>
                </tr>
                <tr className="hover:bg-slate-50/60">
                  <td className="p-3 sm:p-4 font-bold text-slate-900">Cloud Server Upload</td>
                  <td className="p-3 sm:p-4 text-indigo-700 font-bold">0 KB (Zero Uploads)</td>
                  <td className="p-3 sm:p-4 text-slate-600">Confidential payroll, taxes, and financial figures remain strictly in your browser</td>
                </tr>
                <tr className="hover:bg-slate-50/60">
                  <td className="p-3 sm:p-4 font-bold text-slate-900">Apple ID / Account Needed</td>
                  <td className="p-3 sm:p-4 text-indigo-700 font-bold">No Account Required</td>
                  <td className="p-3 sm:p-4 text-slate-600">Instant direct download with zero sign-up forms, email gates, or captchas</td>
                </tr>
              </tbody>
            </table>
          </div>
